    ...
    <v-list-tile
      ...
      <v-hover>
        <v-card slot-scope="{hover}" class="mx-auto"
                ...
                max-width="600" >
          <div v-if="hover"
               class="d-flex transition-fast-in-fast-out orange darken-2 display-3 white--text v-card--reveal"
               style="height: 100%;"
          >
  ...
  export default {
    ...
    methods: {
      toggle: function () {
        ...
        // 展开后查询子节点
        this.$ajax.get(this.url, {params: {pid: this.model.id}})
          .then(resp => {
      ...
      addChild: function () {
        ...
        if (!this.model.isParent) {
          ...
          if (!this.isFolder) {
            this.$ajax.get(this.url, {params: {pid: this.model.id}}).then(resp => {
              Vue.set(this.model, 'children', resp.data);
